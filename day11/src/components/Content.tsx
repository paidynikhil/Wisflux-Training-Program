...
const Content: FC = (): JSX.Element => {
 ...
 const handleEnter: KeyboardEventHandler<HTMLInputElement> = async (
   e: KeyboardEvent<HTMLInputElement>
 ) => {
   if (e.code !== "Enter") return;
   const inputUserName: string = (e.currentTarget as HTMLInputElement).value;
   (e.currentTarget as HTMLInputElement).value = "";
   setUsername(inputUserName);
   try {
     const myEventResponse: AxiosResponse<Event[]> = await axios.get(
       `https://api.github.com/users/${inputUserName}/events`
     );
     const myUserResponse: AxiosResponse = await axios.get(
